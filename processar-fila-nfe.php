<?php
// Processa a fila de NF-e pendentes de envio à SEFAZ.
// Pode rodar via navegador (admin, botão manual) ou via CLI/cron:
//   php processar-fila-nfe.php --cli
...
function buscarNotasPendentesNfe(PDO $dbNotas): array
{
    $stmt = $dbNotas->query(
        "SELECT n.id, n.numero_interno, n.serie, n.empresa_emissora_id, n.cliente_id, n.funcionario_id, n.ambiente,
                n.natureza_operacao, n.data_emissao, n.valor_total,
                e.razao_social AS empresa_razao_social
         FROM notas_fiscais n
         INNER JOIN empresas_emissoras e ON e.id = n.empresa_emissora_id
         WHERE n.tipo_nota = 'nfe' AND n.status = 'pendente_envio'
...
function obterLockNotaNfe(PDO $dbNotas, int $notaId): bool
{
...
function liberarLockNotaNfe(PDO $dbNotas, int $notaId): void
{
...
function processarNotaNfe(PDO $dbNotas, array $notaResumo, int $operadorId): array
{
    $notaId = (int) $notaResumo['id'];
    if (!obterLockNotaNfe($dbNotas, $notaId)) {
    ...
    try {
        ...
        $nota = $stmt->fetch();
        if (!$nota || $nota['status'] !== 'pendente_envio' || $nota['tipo_nota'] !== 'nfe') {
        ...
        registrarLogNota($dbNotas, $notaId, $operadorId, 'tentativa_envio', 'Montando e assinando XML da NF-e.');

        $montagem = nfeMontarXml($nota, $empresa, $cliente, $nfeExtra, $itens);
        if (!empty($montagem['erros'])) {
            ...
            $stmt->execute(['motivo' => mb_substr($motivo, 0, 60000), 'id' => $notaId]);
            registrarLogNota($dbNotas, $notaId, $operadorId, 'rejeitada', mb_substr($motivo, 0, 255));
            return ['sucesso' => false, 'status' => 'rejeitada', 'motivo_rejeicao' => $motivo];
        }

        $resultado = nfeAssinarEEnviar($montagem['xml'], $empresa);
        ...
        liberarLockNotaNfe($dbNotas, $notaId);
    }
}
...
if ($viaCli) {
    ...
    $pendentes = buscarNotasPendentesNfe($dbNotas);
    echo count($pendentes) . " nota(s) pendente(s) de envio.\n";

    foreach ($pendentes as $notaResumo) {
        $resultado = processarNotaNfe($dbNotas, $notaResumo, 0);
...
try {
    ...
    if (empty($_SESSION['csrf_processar_fila_nfe'])) {
        $_SESSION['csrf_processar_fila_nfe'] = bin2hex(random_bytes(32));
    }

    if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['acao'] ?? '') === 'processar') {
        $csrf = $_POST['csrf'] ?? '';
        if (!hash_equals($_SESSION['csrf_processar_fila_nfe'], $csrf)) {
            $erro = 'Sessão expirada. Atualize a página e tente novamente.';
        } else {
            $pendentes = buscarNotasPendentesNfe($dbNotas);
            foreach ($pendentes as $notaResumo) {
                $resultado = processarNotaNfe($dbNotas, $notaResumo, $funcionarioId);
                $resultados[] = [
    ...
    $pendentes = buscarNotasPendentesNfe($dbNotas);
    [$integracaoDisponivel, $motivoIndisponivel] = integracaoNfeDisponivel();
} catch (PDOException $e) {
    $erro = 'Erro ao carregar a fila de NF-e: ' . $e->getMessage();
    $pendentes = [];
    $integracaoDisponivel = false;
    $motivoIndisponivel = '';
}

$csrf = h($_SESSION['csrf_processar_fila_nfe'] ?? '');
?>
<!DOCTYPE html>
<html lang="pt-BR">
<head>
    ...
    <title>Processar Fila de NF-e | ACCOUNT Contabilidade</title>
...
<body>
    <div class="shell">
        ...
        <section class="panel">
            <h1>Fila de NF-e</h1>
            <p class="muted">Processa as notas de produto marcadas como "pendente de envio", tentando transmitir cada uma à SEFAZ do estado da empresa emissora.</p>
        </section>
        ...
        <?php if (!$integracaoDisponivel): ?>
            <div class="notice warning">
                ...
                As notas continuam acumulando aqui como "pendente de envio" até as dependências do Composer, as extensões PHP (soap/curl) e o certificado digital A1 da empresa serem configurados.
            </div>
        <?php endif; ?>
