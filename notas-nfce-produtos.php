<?php
/**
 * Catálogo de produtos da NFC-e — separado do catálogo de NF-e/NFS-e
 * (notas-produtos-servicos.php, não alterado). Usa a mesma tabela
 * notas_produtos_servicos, mas em linhas próprias com tipo='nfce' (ver
 * includes/notas-nfce-schema.php::prepararEnumTipoProdutoNfce()), então os dois catálogos
 * nunca se misturam nem nas telas nem nas consultas.
 */
...
function formatarNcmExibicaoNfceProdutos(?string $ncm): string
{
...
try {
    ...
    if (empty($_SESSION['csrf_notas_nfce_produtos'])) {
        $_SESSION['csrf_notas_nfce_produtos'] = bin2hex(random_bytes(32));
    }

    // Mesmo padrão de notas-produtos-servicos.php: catálogo travado na empresa ativa da sessão.
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        $csrf = $_POST['csrf'] ?? '';
        if (!hash_equals($_SESSION['csrf_notas_nfce_produtos'], $csrf)) {
            ...
            if ($empresaEmissoraAtivaId <= 0 || $descricao === '') {
                ...
                $stmt = $dbNotas->prepare(
                    'INSERT INTO notas_produtos_servicos (
                        ...
                        unidade, valor_unitario_padrao, aliquota_icms, aliquota_pis, aliquota_cofins,
                        ipi_cst, aliquota_ipi, cean, icms_origem, cest, cnpj_fabricante, indicador_escala_relevante,
                        codigo_beneficio_fiscal, ativo
                     ) VALUES (
                        :empresa_emissora_id, \'nfce\', :descricao, :codigo_interno, :ncm, :cfop, :cst_csosn,
                        :unidade, :valor_unitario_padrao, :aliquota_icms, :aliquota_pis, :aliquota_cofins,
                        :ipi_cst, :aliquota_ipi, :cean, :icms_origem, :cest, :cnpj_fabricante, :indicador_escala_relevante,
                ...
                $sucesso = 'Item cadastrado no catálogo de NFC-e.';
            }
            ...
            if ($itemId <= 0 || $descricao === '') {
                ...
                $stmt = $dbNotas->prepare(
                    'UPDATE notas_produtos_servicos SET
                        descricao = :descricao, codigo_interno = :codigo_interno, ncm = :ncm, cfop = :cfop,
                        cst_csosn = :cst_csosn, unidade = :unidade,
                     ...
                     WHERE id = :id AND empresa_emissora_id = :empresa_emissora_id AND tipo = \'nfce\''
                );
                $stmt->execute([
                    'empresa_emissora_id' => $empresaEmissoraAtivaId,
                    'descricao' => $descricao,
            ...
            if ($id > 0) {
                $stmt = $dbNotas->prepare('UPDATE notas_produtos_servicos SET ativo = 0 WHERE id = :id AND empresa_emissora_id = :empresa_emissora_id AND tipo = \'nfce\'');
            ...
            $id = (int) ($_POST['item_id'] ?? 0);
            if ($id > 0) {
                $stmt = $dbNotas->prepare('UPDATE notas_produtos_servicos SET ativo = 1 WHERE id = :id AND empresa_emissora_id = :empresa_emissora_id AND tipo = \'nfce\'');
                $stmt->execute(['id' => $id, 'empresa_emissora_id' => $empresaEmissoraAtivaId]);
    ...
    $stmt = $dbNotas->prepare(
        'SELECT id, empresa_emissora_id, descricao, codigo_interno, ncm, cfop, cst_csosn, unidade, valor_unitario_padrao,
                aliquota_icms, aliquota_pis, aliquota_cofins, ipi_cst, aliquota_ipi, cean, ativo,
                icms_origem, cest, cnpj_fabricante, indicador_escala_relevante, codigo_beneficio_fiscal
         FROM notas_produtos_servicos
         WHERE empresa_emissora_id = :empresa_emissora_id AND tipo = \'nfce\'
...
$csrf = h($_SESSION['csrf_notas_nfce_produtos'] ?? '');
?>
<!DOCTYPE html>
<html lang="pt-BR">
<head>
    ...
    <title>Produtos NFC-e | ACCOUNT Contabilidade</title>
...
<body>
    <div class="shell">
        <?php $paginaAtivaNotasNfce = 'produtos'; include __DIR__ . '/includes/notas-nfce-nav.php'; ?>

        <section class="panel">
            <h1>Produtos NFC-e</h1>
            <p class="muted">Catálogo próprio da venda de balcão — separado do catálogo de Produtos/Serviços da NF-e/NFS-e. Na tela "Emitir NFC-e" só aparecem itens cadastrados aqui.</p>
        </section>
        ...
        <?php if ($empresaEmissoraAtivaId <= 0): ?>
                ...
                <form method="post">
                    ...
                    <div class="form-grid">
                        <div class="field">
                            ...
                            <p class="muted" style="margin-top:0.35rem;font-size:0.78rem;">O item é cadastrado na empresa ativa (selecionada em "Vendendo por", no topo).</p>
                        </div>
                    ...
                    <h3 style="margin-top: 1.5rem;"><i class="fa-solid fa-file-invoice"></i> Fiscal</h3>
                    <div class="form-grid">
                        <div class="field">
                            <label for="ncm">NCM</label>
                            <input id="ncm" name="ncm" type="text" maxlength="10" placeholder="Digite o código ou o nome do produto" inputmode="numeric" list="datalistNcm" autocomplete="off" value="<?php echo h(formatarNcmExibicaoNfceProdutos($itemEmEdicao['ncm'] ?? '')); ?>">
                    ...
                    <div style="margin-top: 1.5rem;" class="row-actions">
                        ...
                        <?php if ($itemEmEdicao): ?>
                            <a class="btn btn-outline" href="notas-nfce-produtos">Cancelar edição</a>
                        <?php endif; ?>
                    </div>
                </form>
            </section>
                            ...
                            <tr>
                                ...
                                <td>
                                    <div class="row-actions">
                                        <a class="btn btn-outline" href="notas-nfce-produtos?editar=<?php echo h((string) $item['id']); ?>"><i class="fa-solid fa-pen-to-square"></i> Editar</a>
                                        <form method="post">
                                            <input type="hidden" name="csrf" value="<?php echo $csrf; ?>">
    ...
    <script>
        function formatarNcmCatalogoNfce(valor) {
        ...
        function escaparHtmlCatalogoNfce(texto) {
            return String(texto).replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/"/g, '&quot;');
        }

        let ncmCodigosCatalogoNfce = [];
        fetch('ncm-codigos.json', { cache: 'force-cache' })
            .then(function (resposta) { return resposta.json(); })
            .then(function (dados) { ncmCodigosCatalogoNfce = dados; })
            .catch(function () { ncmCodigosCatalogoNfce = []; });

        const datalistNcmCatalogoNfce = document.getElementById('datalistNcm');
        let buscaNcmCatalogoNfceTimeout = null;
        function buscarNcmCatalogoNfce(textoDigitado) {
            if (!datalistNcmCatalogoNfce) return;
            clearTimeout(buscaNcmCatalogoNfceTimeout);
            buscaNcmCatalogoNfceTimeout = setTimeout(function () {
                const termo = (textoDigitado || '').trim().toLowerCase();
                if (termo.length < 2) {
                    datalistNcmCatalogoNfce.innerHTML = '';
                    return;
                }
                const termoDigitos = termo.replace(/\D/g, '');
                const encontrados = [];
                for (let i = 0; i < ncmCodigosCatalogoNfce.length && encontrados.length < 40; i++) {
                    const codigo = ncmCodigosCatalogoNfce[i][0];
                    const descricao = ncmCodigosCatalogoNfce[i][1];
                ...
                datalistNcmCatalogoNfce.innerHTML = encontrados.map(function (item) {
                    return '<option value="' + item[0] + '">' + item[0] + ' - ' + escaparHtmlCatalogoNfce(item[1]) + '</option>';
                }).join('');
            }, 200);
        }

        const campoNcmCatalogoNfce = document.getElementById('ncm');
        if (campoNcmCatalogoNfce) {
            campoNcmCatalogoNfce.addEventListener('input', function () {
                if (/^[\d.]*$/.test(campoNcmCatalogoNfce.value)) {
                    campoNcmCatalogoNfce.value = formatarNcmCatalogoNfce(campoNcmCatalogoNfce.value);
                }
                buscarNcmCatalogoNfce(campoNcmCatalogoNfce.value);
            });
        }
